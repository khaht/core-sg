...
const checkDuplicatedTableNames = ({ sgApp }) => {
  const modelsWithInfo = [];
  // _.forIn(sgApp.admin.models, (model, modelName) => {
  //   modelsWithInfo.push({
  //     origin: 'sgApp internal',
  //     model,
  ...
  _.forIn(sgApp.plugins, (plugin, pluginName) => {
  ...
  modelsWithInfo.forEach((modelA) => {
    const similarModelFound = modelsWithInfo.find(
      (modelB) => modelB.model.collectionName === modelA.model.collectionName
